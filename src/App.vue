...
<template>
  <div class="bg-black min-h-screen text-white">
    ...
    <transition name="notif-pop">
      <div v-if="customToast" @click="router.push(`/product/${customToast.productId}`); customToast = null;"
      ...
      >
        <div class="w-10 h-10 bg-yellow-500 rounded-2xl flex items-center justify-center shrink-0">
          <BellIcon class="w-5 h-5 text-black" />
        </div>
        <div class="flex-1 min-w-0">
          <p class="text-[9px] font-black text-yellow-500 uppercase italic tracking-widest">{{ customToast.title }}</p>
          <p class="text-[11px] font-bold text-white uppercase truncate italic">{{ customToast.message }}</p>
        </div>
        <XMarkIcon @click.stop="customToast = null" class="w-5 h-5 text-gray-600 hover:text-white" />
      </div>
    </transition>

    <Header v-if="!$route.meta.requiresAdmin" :userProfile="userProfile" :authReady="authReady" />
...
<style>
.notif-pop-enter-active { transition: all 0.5s cubic-bezier(0.175, 0.885, 0.32, 1.275); }
.notif-pop-leave-active { transition: all 0.3s ease-in; }
.notif-pop-enter-from { transform: translateY(-100px); opacity: 0; }
.notif-pop-leave-to { transform: translateY(-100px); opacity: 0; }
