...
const userRank = computed(() => {
  if (props.userProfile?.is_admin === true) {
    return { name: "OWNER", limit: Infinity, color: "text-yellow-500 drop-shadow-[0_0_10px_rgba(234,179,8,0.8)] font-black" };
  }
...
const submitReport = async () => {
  ...
  try {
    ...
    notify.success("Laporan Terkirim", "Laporan sedang diproses.");
  } catch (err) {
...
const confirmPayment = async (method) => {
  ...
  if (!error) {
    ...
    notify.success("Berhasil", "Dana diamankan di Escrow.");
...
const closeAuctionOfficial = async () => {
  ...
  try {
    ...
    if (!error) {
      product.value.status = "closed";
      notify.success("LELANG SELESAI", "Sesi transmisi ditutup.");
      fetchTransaction();
    }
...
const updateTimer = () => {
  ...
  if (diff <= 0) {
    if (product.value.status === 'closed') {
      ...
      // Auto-terminator jika mampet 5 detik
      if (diff < -5000) closeAuctionOfficial();
    }
  ...
  if (diff > 0 && diff <= 120000) {
    ...
    if (!hasNotifiedIntense.value) {
      notify.error("WAR ZONE!", "Waktu kritis 2 menit!");
...
// --- PLACE BID (RPC AUTHORITY) ---
const placeBid = async () => {
  if (!props.userProfile) return notify.error("Auth Required", "Login dulu bosku!");
  if (isCooldown.value) return;

  // HAK HAKIM: CEK STATUS DATABASE, BUKAN TIMER
  ...
  if (bidAmount.value > userRank.value.limit) {
    return notify.error("Limit Rank", `Bid melebihi limit rank Anda.`);
  }
...
onMounted(() => {
  ...
  timerInterval = setInterval(() => {
    updateTimer();
    if (product.value?.status === "closed" && !transaction.value) fetchTransaction();
  }, 1000);
  ...
  if (route.params.id) {
    bidSubscription = supabase
      ...
      .on("postgres_changes", { event: "INSERT", schema: "public", table: "bids", filter: `product_id=eq.${route.params.id}` }, (payload) => {
          fetchBids(); 
          const nextVal = Number(payload.new.amount) + 10000;
          if (bidAmount.value < nextVal) bidAmount.value = nextVal;
      })
...
onUnmounted(() => { clearInterval(timerInterval); if (bidSubscription) supabase.removeChannel(bidSubscription); });
</script>
      ...
      <div class="grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-12 items-start">
        ...
        <div class="lg:col-span-7 space-y-10">
          <div class="relative w-full aspect-square lg:aspect-video mt-4 overflow-visible" @touchstart="handleTouchStart" @touchend="handleTouchEnd">
            <div class="relative w-full h-full" @click="nextImage">
              ...
              <div class="hidden lg:flex absolute bottom-6 left-6 z-[50] backdrop-blur-md px-5 py-2.5 rounded-2xl border items-center gap-3 transition-all duration-500" :class="isIntense ? 'bg-red-600 border-red-500 shadow-xl' : 'bg-black/60 border-white/10'">
                <ClockIcon class="w-5 h-5" :class="isIntense ? 'text-white animate-spin' : 'text-yellow-500 animate-pulse'" />
                <span class="text-xs font-[1000] italic text-white">{{ timeLeft }}</span>
              </div>
            </div>
          </div>
          ...
          <div class="hidden lg:block space-y-6">
            <div class="flex p-1.5 bg-white/5 border border-white/10 rounded-2xl w-full max-w-xs mb-6">
              <button @click="activeBidTab = 'ranking'" :class="activeBidTab === 'ranking' ? 'bg-yellow-500 text-black shadow-lg' : 'text-gray-500 hover:text-white'" class="flex-1 py-3 rounded-xl text-[10px] font-black uppercase italic transition-all cursor-pointer">Ranking</button>
              <button @click="activeBidTab = 'history'" :class="activeBidTab === 'history' ? 'bg-white text-black shadow-lg' : 'text-gray-500 hover:text-white'" class="flex-1 py-3 rounded-xl text-[10px] font-black uppercase italic transition-all cursor-pointer">History</button>
            </div>
                ...
                <div v-for="(bid, index) in rankedBids" :key="'rank-' + bid.id" @click="router.push(`/user/${bid.profiles?.username}`)" class="flex items-center justify-between p-5 rounded-[28px] border border-white/5 bg-white/[0.02] group cursor-pointer hover:border-yellow-500/30 transition-all shadow-xl" :class="index === 0 ? 'border-yellow-500/30 bg-yellow-500/5 ring-1 ring-yellow-500/20' : ''">
                  <div class="flex items-center gap-5">
                    ...
                    <div class="w-12 h-12 rounded-2xl overflow-hidden border-2 border-white/10 flex-shrink-0">
                      ...
                      <div v-else class="w-full h-full bg-gray-800 flex items-center justify-center"><UserIcon class="w-6 h-6 text-gray-500" /></div>
                    </div>
                    <div>
        ...
        <div class="lg:col-span-5 space-y-8">
          ...
          <div class="bg-[#0a0a0a] border border-white/10 rounded-[45px] p-8 shadow-2xl relative overflow-hidden">
            ...
            <h3 class="text-3xl sm:text-5xl lg:text-6xl font-[1000] italic text-yellow-500 tracking-tighter mb-10 drop-shadow-[0_0_30px_rgba(234,179,8,0.3)] break-all leading-none">
                ...
                <div v-for="(bid, index) in rankedBids" :key="'mb-rank-' + bid.id" @click="router.push(`/user/${bid.profiles?.username}`)" class="flex items-center justify-between p-4 rounded-2xl border border-white/5 bg-white/[0.02] cursor-pointer" :class="index === 0 ? 'border-yellow-500/20 bg-yellow-500/5 shadow-xl' : ''">
                  <div class="flex items-center gap-3">
                    ...
                    <div class="w-10 h-10 rounded-xl overflow-hidden border border-white/10 flex-shrink-0">
                      <img v-if="bid.profiles?.avatar_url && bid.profiles.avatar_url.trim() !== ''" :src="bid.profiles.avatar_url" class="w-full h-full object-cover" />
                      <div v-else class="w-full h-full bg-gray-800 flex items-center justify-center"><UserIcon class="w-6 h-6 text-gray-500 p-2" /></div>
                    </div>
    ...
    <div v-if="showReportModal" class="fixed inset-0 z-[200] flex items-center justify-center px-6">
      ...
      <div class="relative w-full max-w-md bg-[#0d0d0d] border border-white/10 rounded-[40px] p-10 shadow-2xl overflow-hidden text-center">
        ...
        <div class="space-y-6 text-left">
          <select v-model="reportForm.category" class="w-full bg-black border border-white/10 rounded-2xl p-4 text-xs font-bold text-white outline-none focus:border-red-500 appearance-none">
