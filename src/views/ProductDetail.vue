...
const fetchProductDetail = async () => {
  ...
  try {
    const { data, error } = await supabase
      .from("products")
      .select("*, profiles!owner_id(username, full_name, avatar_url, reputation)")
        ...
        <div class="lg:col-span-7 space-y-10">
          <div class="relative w-full aspect-square lg:aspect-video mt-4 overflow-visible" @touchstart="handleTouchStart" @touchend="handleTouchEnd">
            <div class="relative w-full h-full" @click="nextImage">
              ...
              <div class="hidden lg:flex absolute bottom-6 left-6 z-[50] backdrop-blur-md px-5 py-2.5 rounded-2xl border items-center gap-3 transition-all duration-500" :class="isIntense ? 'bg-red-600 border-red-500 shadow-[0_0_20px_rgba(239,68,68,0.5)] scale-110' : 'bg-black/60 border-white/10'">
                <ClockIcon class="w-5 h-5" :class="isIntense ? 'text-white animate-spin' : 'text-yellow-500 animate-pulse'" />
                <span class="text-xs font-[1000] italic tracking-[0.2em] uppercase text-white">{{ timeLeft }}</span>
              </div>
            </div>
          </div>
          ...
          <div class="hidden lg:block space-y-6">
            <div class="px-2">
              ...
              <div class="flex p-1.5 bg-white/5 border border-white/10 rounded-2xl w-full max-w-xs mb-6">
                <button @click="activeBidTab = 'ranking'" :class="activeBidTab === 'ranking' ? 'bg-yellow-500 text-black shadow-lg' : 'text-gray-500'" class="flex-1 py-3 rounded-xl text-[10px] font-black uppercase italic transition-all duration-300">Ranking</button>
                <button @click="activeBidTab = 'history'" :class="activeBidTab === 'history' ? 'bg-white text-black shadow-lg' : 'text-gray-500'" class="flex-1 py-3 rounded-xl text-[10px] font-black uppercase italic transition-all duration-300">History</button>
              </div>
            </div>
                ...
                <div v-for="bid in recentBids" :key="'hist-' + bid.id" class="flex items-center justify-between p-4 rounded-2xl border border-white/5 bg-black/40">
                  <div class="flex items-center gap-4">
                    ...
                    <div>
                      ...
                      <p class="text-[8px] text-gray-700 font-bold uppercase mt-0.5">{{ new Date(bid.created_at).toLocaleTimeString() }}</p>
                    </div>
                  </div>
        ...
        <div class="lg:col-span-5 space-y-8">
          ...
          <div class="bg-[#0a0a0a] border border-white/10 rounded-[45px] p-8 shadow-2xl relative overflow-hidden">
            <div class="flex items-center justify-between mb-4">
              <p class="text-[10px] font-black text-gray-500 uppercase italic tracking-widest">Hammer Price</p>
              <div class="flex items-center gap-2">
                <div class="w-2 h-2 rounded-full bg-red-600 animate-pulse"></div>
                <span class="text-[8px] font-black text-red-500 uppercase italic">Live Mode</span>
              </div>
            </div>
            <h3 class="text-4xl lg:text-7xl font-[1000] italic text-yellow-500 tracking-tighter mb-10 drop-shadow-[0_0_30px_rgba(234,179,8,0.3)]">
    ...
    <div v-if="showReportModal" class="fixed inset-0 z-[200] flex items-center justify-center px-6">
      ...
      <div class="relative w-full max-w-md bg-[#0d0d0d] border border-white/10 rounded-[40px] p-10 shadow-2xl overflow-hidden text-center">
        ...
        <div class="space-y-6 text-left">
          <div><label class="text-[9px] font-black text-gray-600 uppercase block mb-3 italic">Reason Category</label>
            <select v-model="reportForm.category" class="w-full bg-black border border-white/10 rounded-2xl p-4 text-xs font-bold text-white outline-none focus:border-red-500">
