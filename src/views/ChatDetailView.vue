...
// 3. LOGIKA ONLINE DIAMBIL DARI PINIA (SANGAT ENTENG)
// Gunakan watchEffect agar isTargetOnline otomatis berubah saat Pinia update
import { watchEffect } from "vue";
watchEffect(() => {
  isTargetOnline.value = presenceStore.isUserOnline(targetId);
});
...
<template>
  <div
    class="fixed inset-0 z-[999] bg-[#050505] flex justify-center overflow-hidden overscroll-none"
  >
    <div
      class="relative w-full max-w-2xl bg-[#0a0a0a] flex flex-col h-full border-x border-white/5 shadow-2xl"
    >
      <header
        class="h-16 shrink-0 z-30 bg-[#0a0a0a]/95 backdrop-blur-md border-b border-white/5 px-4 flex items-center justify-between"
      >
        <div class="flex items-center gap-3">
          <button
            @click="router.back()"
            class="p-2 bg-white/5 rounded-lg border border-white/10 text-gray-400 active:scale-90"
          >
          ...
          >
            ...
            <div>
              <h2
                class="text-[11px] font-[1000] text-white uppercase italic leading-none mb-1 group-hover:text-yellow-500 transition-colors"
              >
          ...
          >
            ...
            >
              ...
              <div class="flex items-center justify-end gap-1 mt-1.5">
                <p
                  class="text-[6px] opacity-40 uppercase font-black tracking-tighter"
                >
                  {{
