      ...
      <template v-else>
        <section class="max-w-[1440px] mx-auto px-6">
          ...
          <div ref="priorityContainer" class="carousel-wrapper no-scrollbar">
            ...
            <div v-if="isSlotAvailable" class="card-container min-h-[300px]">
              <div class="h-full border border-dashed border-yellow-500/20 bg-yellow-500/[0.02] rounded-[32px] flex flex-col items-center justify-center text-center p-6 transition-all hover:bg-yellow-500/[0.05] group">
                <div class="w-10 h-10 mb-3 bg-yellow-500/10 rounded-xl flex items-center justify-center group-hover:scale-110 transition-transform">
                  <PlusIcon class="w-5 h-5 text-yellow-500" />
                </div>
                <h3 class="text-yellow-500 font-black italic uppercase tracking-widest text-[10px] mb-1">Slot Iklan Tersedia</h3>
                <p class="text-gray-500 text-[8px] uppercase tracking-widest leading-relaxed mb-4">Tampilkan barangmu di sini</p>
                <a href="https://wa.me/xxx" class="bg-yellow-500/10 hover:bg-yellow-500 text-yellow-500 hover:text-black px-4 py-2 rounded-lg text-[8px] font-black uppercase transition-all">Hubungi Admin</a>
              </div>
            </div>
          </div>
        </section>
        ...
        <section class="max-w-[1440px] mx-auto px-6">
          <div class="flex items-center justify-between mb-8">
            ...
            <div v-if="regularProducts.length > 0" class="hidden md:flex items-center space-x-3">
              <button @click="scroll(scrollContainer, 'left')" class="p-2.5 bg-white/5 hover:bg-yellow-500 hover:text-black text-white rounded-xl border border-white/5 transition-all"><ChevronLeftIcon class="w-5 h-5" /></button>
              <button @click="scroll(scrollContainer, 'right')" class="p-2.5 bg-white/5 hover:bg-yellow-500 hover:text-black text-white rounded-xl border border-white/5 transition-all"><ChevronRightIcon class="w-5 h-5" /></button>
            </div>
          </div>

          <div v-if="regularProducts.length > 0" ref="scrollContainer" class="carousel-wrapper no-scrollbar">
            <div v-for="product in regularProducts" :key="product.id" class="card-container"><AuctionCard :product="product" /></div>
          </div>

          <div v-else class="py-20 text-center border border-white/5 bg-white/[0.02] rounded-[40px]">
            <p class="text-gray-600 font-black uppercase italic tracking-[0.3em] text-[9px]">No Active Transmissions Found</p>
          </div>
        </section>
      </template>
    </div>
