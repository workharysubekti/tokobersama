      ...
      <div class="flex flex-col lg:flex-row gap-10 items-center lg:items-start">
        ...
        <div class="w-full lg:w-1/3 flex flex-col items-center text-center space-y-6">
          ...
          <div class="relative group mt-4">
            <div class="w-32 h-32 lg:w-40 lg:h-40 rounded-full border-4 border-yellow-500/10 overflow-hidden bg-black shadow-2xl">
              <img :src="userProfile.avatar_url || 'https://via.placeholder.com/150'" class="w-full h-full object-cover transition-transform group-hover:scale-110" />
            </div>
          ...
          <div class="space-y-1">
            <h1 class="text-3xl lg:text-4xl tracking-tighter text-white leading-none">
              {{ userProfile.full_name || "MEMBER" }}
            </h1>
            <p class="text-[10px] text-yellow-500/40 tracking-[0.4em]">@{{ userProfile.username }}</p>
          </div>

          <router-link to="/reputation-info" class="inline-flex items-center gap-4 px-6 py-3 bg-yellow-500/5 border border-yellow-500/20 rounded-[24px] active:scale-95 transition-all w-fit">
            <div :class="[userRank.bg, userRank.color]" class="p-2 rounded-xl border border-white/5">
              <component :is="userRank.icon" class="w-4 h-4" />
            </div>
            <div class="text-left">
              <p class="text-[9px] text-white leading-none mb-1 font-black">RANK: {{ userRank.name }}</p>
              <p class="text-[7px] text-yellow-500/50 tracking-widest italic uppercase leading-none">Bonus & Aturan</p>
            </div>
            <ChevronRightIcon class="w-4 h-4 text-yellow-500/30" />
          </router-link>

          <div class="flex items-center gap-4 text-[10px] tracking-widest text-gray-600">
          ...
          <button @click="isEditing = !isEditing" class="text-[9px] text-yellow-500 tracking-[0.2em] font-black border border-yellow-500/10 px-6 py-2 rounded-full">
        ...
        <div class="w-full lg:w-2/3 space-y-8">
          ...
          <div v-if="isEditing" class="bg-yellow-500/[0.02] border border-yellow-500/10 rounded-[32px] p-6 space-y-4">
          ...
          <div class="grid grid-cols-2 gap-4 lg:gap-6">
            <router-link to="/wallet" class="bg-white/[0.02] border border-white/5 rounded-[32px] p-6 active:bg-white/[0.05] transition-all flex flex-col justify-between aspect-square lg:aspect-auto lg:h-40">
              ...
              <div>
                ...
                <p class="text-base lg:text-xl font-[1000] text-white">Rp {{ balance.toLocaleString() }}</p>
              </div>
            </router-link>

            <router-link :to="`/user/${userProfile.username}`" class="bg-white/[0.02] border border-white/5 rounded-[32px] p-6 active:bg-white/[0.05] transition-all flex flex-col justify-between aspect-square lg:aspect-auto lg:h-40">
              <div class="flex justify-between items-start">
              ...
              <div>
                ...
                <p class="text-base lg:text-xl font-[1000] text-white">PROFIL <span class="text-gray-600">PUBLIK</span></p>
              </div>
            </router-link>
          </div>
