...
const openDropdown = ref(null); // Tracking dropdown yang buka
...
const toggleElite = async (product) => {
  ...
  try {
    ...
    openDropdown.value = null; // Tutup dropdown
...
onMounted(() => {
  ...
  // Close dropdown pas klik luar (Buat navigasi HP lebih enak)
            ...
            >
              <td class="px-8 py-6">
                <div class="flex items-center gap-4">
                  <div class="relative">
                    ...
                    />
                    <div v-if="item.is_elite" class="absolute -top-2 -right-2 bg-yellow-500 p-1 rounded-full shadow-[0_0_10px_rgba(234,179,8,0.5)] border-2 border-black">
                      <StarIcon class="w-2.5 h-2.5 text-black" />
                    </div>
                  </div>
                  <div>
                    <p class="text-xs font-black italic uppercase tracking-tighter flex items-center gap-2">
                      {{ item.name }}
                      <span v-if="item.is_elite" class="text-[7px] text-yellow-500 tracking-widest">[ELITE]</span>
                    </p>
                    <p class="text-[8px] font-bold text-gray-500 uppercase italic">
              ...
              <td class="px-8 py-6">
                <div v-if="item.status === 'rejected'" class="flex items-center gap-2 text-red-500/50">
                  ...
                  <span class="text-[9px] font-black uppercase italic tracking-widest text-gray-500">Rejected / Terminated</span>
                </div>

                <div v-else-if="isEnded(item.end_time)" class="flex items-center gap-2 text-orange-500 opacity-60">
                  <CheckBadgeIcon class="w-3 h-3" />
                  <span class="text-[9px] font-black uppercase italic tracking-widest">Auction Ended</span>
                </div>
                ...
                <div v-else class="flex items-center gap-2 text-green-500">
                  ...
                  <span class="text-[9px] font-black uppercase italic tracking-widest">Live Bidding</span>
                </div>
              </td>
              ...
              <td class="px-8 py-6 text-right relative dropdown-area">
                <button 
                  ...
                  class="p-2.5 hover:bg-white/5 rounded-xl transition-all text-gray-500 hover:text-white"
                >
                ...
                <div 
                  ...
                  class="absolute right-8 mt-2 w-48 bg-[#111] border border-white/10 rounded-2xl shadow-2xl z-[100] overflow-hidden py-1"
                >
                  <router-link :to="'/product/' + item.id" class="w-full px-4 py-3 text-left text-[9px] font-black italic uppercase flex items-center gap-3 hover:bg-white/5 text-gray-400 hover:text-white transition-all">
                    <EyeIcon class="w-4 h-4" /> View Transmission
                  </router-link>
                  ...
                  <button 
                    ...
                    :class="item.is_elite ? 'text-red-400' : 'text-yellow-500'"
                  >
                    <ArrowUpCircleIcon v-if="!item.is_elite" class="w-4 h-4" />
                    <StarIcon v-else class="w-4 h-4" />
                    {{ item.is_elite ? 'Remove from Elite' : 'Promote to Elite' }}
                  </button>
...
<style scoped>
/* Biar scrollbar horizontal ga ngerusak look */
