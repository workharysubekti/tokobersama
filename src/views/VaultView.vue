...
<template>
  <div class="min-h-screen bg-[#050505] text-white pt-12 px-5 pb-32">
    <div class="max-w-2xl mx-auto">
      <div class="flex flex-col mb-10">
        <div class="flex items-center gap-3 mb-2">
          <ArchiveBoxIcon class="w-6 h-6 text-yellow-500" />
          <h2
            class="text-3xl font-[1000] italic uppercase tracking-tighter text-white"
          >
            The <span class="text-yellow-500">Vault</span>
          </h2>
        </div>
        <p
          class="text-[9px] font-black uppercase tracking-[0.3em] text-gray-600 italic ml-9"
        >
          Storage System & Item History
        </p>
      </div>

      <div
        class="flex bg-white/[0.03] border border-white/5 p-1.5 rounded-[22px] mb-8"
      >
        <button
        ...
        <button
          ...
          :class="
            activeTab === 'listings' ? 'bg-white text-black' : 'text-gray-500'
          "
          class="flex-1 flex items-center justify-center gap-2 py-3 rounded-[18px] text-[10px] font-black uppercase italic transition-all duration-300"
        >
      ...
      >
        <ArrowPathIcon
          class="w-10 h-10 text-yellow-500 animate-spin opacity-30"
        />
      </div>
      ...
      <div v-else>
        <div v-if="activeTab === 'collection'" class="space-y-4">
          ...
          <div
            v-else
            class="py-20 text-center border-2 border-dashed border-white/5 rounded-[35px]"
          >
